...
def _train(training_cfg):
    ...
    if training_cfg.loss == "sft":
        dataset = Dataset.from_list(
            [dict(messages=r["messages"]) for r in rows]
        )
    else:
    ...
    if training_cfg.intervention:
        ...
        with open(
            os.path.join(training_cfg.save_path, "intervention_shapes.json"),
            "w",
        ) as f:
...
def push_model(training_cfg, finetuned_model_id, model, tokenizer):
    if training_cfg.push_to_hub:
        ...
        if training_cfg.merge_before_push:
            print(
                "Merge not supported when push_to_hub is False. Saving only adapter weights."
            )
        model.save_pretrained(training_cfg.save_path)
        tokenizer.save_pretrained(training_cfg.save_path)
        # save the training config
        with open(
            os.path.join(training_cfg.save_path, "training_config.json"), "w"
        ) as f:
